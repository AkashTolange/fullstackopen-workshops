...
const Appstyle = () => {
  ...
  useEffect(() => {
    ...
    myAxiosPromise
      ...
      .catch((err) => {
        ...
        setNotification(
          "failed to load notes. Please check the server connection "
        );
    ...
    if(myUser){ 
      setUser(myUser ? JSON.parse(myUser) : null);
    }
    ...
    postPromise
      .then((createdNote) => {
        ...
        if (createdNote.data) {
          setNotes(notes.concat(createdNote.data));
          //here added 
          setNotification("Note successfully added");
          setTimeout(() => {
            setNotification("");
          }, 2000);  //msg dispaears after 2 sec
        } else {
          console.error(
            "Created note data is not in expected format:",
            createdNote.data
          );
        }
        // setNewNote("");
      ...
      .catch((error) => {
        setNotification(
          error.response ? error.response.data.error : "Error creating note"
        );
        setTimeout(() => {
          setNotification("");
        }, 2000);
    ...
    putPromise
      ...
      .catch((err) => {
        ...
        if (err.response && err.response.status === 404) {
          ...
          setNotification(
            `sorry this note is ${currentNote.content} does not exist`
          );
          setTimeout(() => {
            setNotification("");
          }, 2000);
  ...
  const loginForm = () => {
    return (
        // <h1>just a test looking akash</h1>
  ...
  return (
    <>
      <h2 style={stylevariable} className="redbackground">
        hello there Notes{" "}
      </h2>
      ...
      {/* we need a login form from here  */}
      {/* <h1>Login Form</h1> */}
      {/* ternay operator */}
      ...
      <ul>
        {notesToShow.map((value) => {
          //value is now object and will run for each object in array ok
          return (
      ...
      {/*//what is event and event.target look below */}
